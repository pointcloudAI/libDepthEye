...
namespace PointCloud
{
    ...
    class SYMBOL_EXPORT PhaAmpFrame : public Frame
    {
    public:
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *t = static_cast<PhaAmpFrame *>(other.get());
        ...
        {
            const PhaAmpFrame *f = static_cast<const PhaAmpFrame *>(&other);
            return f;
        }

        static Ptr<PhaAmpFrame> typeCast(FramePtr ptr)
        {
            return std::static_pointer_cast<PhaAmpFrame>(ptr);
        }
        ...
        {
            size_t s = sizeof(id) + sizeof(type) + sizeof(timestamp) +
    ...
    class SYMBOL_EXPORT BufferFrame : public Frame
    {
    public:
        uint8_t *dataPtr = 0;
        size_t length = 0;
        ...
        {
            if (!other || !isSameType(*other))
                other = Ptr<Frame>(new BufferFrame());

            auto *t = static_cast<BufferFrame *>(other.get());
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const BufferFrame *f = static_cast<const BufferFrame *>(&other);
            return f;
        }

        static Ptr<BufferFrame> typeCast(FramePtr ptr)
        {
            return std::static_pointer_cast<BufferFrame>(ptr);
        }
        ...
        virtual bool serialize(SerializedObject &object) const
        {
            size_t s = sizeof(id) + sizeof(type) + sizeof(timestamp) + sizeof(size_t) * 2 +
                       sizeof(buff_idx) + sizeof(length) + length;
            object.resize(s);
            object.put((const char *)&id, sizeof(id));
            object.put((const char *)&type, sizeof(type));
            object.put((const char *)&timestamp, sizeof(timestamp));
            object.put((const char *)&size.width, sizeof(size.width));
            object.put((const char *)&size.height, sizeof(size.height));
            object.put((const char *)&buff_idx, sizeof(buff_idx));
            object.put((const char *)&length, sizeof(length));
            object.put((const char *)dataPtr, length);
            return true;
        }
        ...
        virtual bool deserialize(SerializedObject &object)
        {
            ...
            object.get((char *)&size.width, sizeof(size.width));
            object.get((char *)&size.height, sizeof(size.height));
            object.get((char *)&buff_idx, sizeof(buff_idx));
            object.get((char *)&length, sizeof(length));
            unsigned int frameSize = size.width * size.height;
            object.get((char *)dataPtr, length);

            return true;
        }
    };

    typedef Ptr<BufferFrame> BufferFramePtr;
    class SYMBOL_EXPORT VerticalDepthFrame : public Frame
    {
    public:
        uint8_t *dataPtr;
        size_t length;
        FrameSize size;
        virtual ~VerticalDepthFrame() {}
        virtual int16_t *amplitude() const
        {
            int16_t offset = size.width * size.height;
            if (offset * sizeof(int16_t) * 4 != length)
                return 0;
            return (int16_t *)dataPtr;
        }
        virtual int16_t *x_axes() const
        {
            int16_t offset = size.width * size.height;
            if (offset * sizeof(int16_t) * 4 != length)
                return 0;
            return (int16_t *)dataPtr + offset;
        }

        virtual int16_t *y_axes() const
        {
            int16_t offset = size.width * size.height;
            if (offset * sizeof(int16_t) * 4 != length)
                return 0;
            return (int16_t *)dataPtr + offset * 2;
        }

        virtual int16_t *z_axes() const
        {
            int16_t offset = size.width * size.height;
            if (offset * sizeof(int16_t) * 4 != length)
                return 0;
            return (int16_t *)dataPtr + offset * 3;
        }

        virtual Ptr<Frame> copy() const
        {
            Ptr<Frame> f(new VerticalDepthFrame());
            return copyTo(f);
        }
        ...
        {
            if (!other || !isSameType(*other))
                other = Ptr<Frame>(new VerticalDepthFrame());

            auto *t = static_cast<VerticalDepthFrame *>(other.get());
        ...
        virtual Ptr<Frame> newFrame() const
        {
            VerticalDepthFrame *t = new VerticalDepthFrame();
            t->dataPtr = 0;
            t->size = size;
            return FramePtr(t);
        }
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const VerticalDepthFrame *f = static_cast<const VerticalDepthFrame *>(&other);
            return f;
        }

        static Ptr<VerticalDepthFrame> typeCast(FramePtr ptr)
        {
            return std::static_pointer_cast<VerticalDepthFrame>(ptr);
        }
        ...
        virtual bool serialize(SerializedObject &object) const
        {
            size_t s = sizeof(id) + sizeof(type) + sizeof(timestamp) + sizeof(size_t) * 2 + sizeof(length) + length;
            object.resize(s);
            object.put((const char *)&id, sizeof(id));
            object.put((const char *)&type, sizeof(type));
            object.put((const char *)&timestamp, sizeof(timestamp));
    ...
    typedef Ptr<VerticalDepthFrame> VerticalDepthFramePtr;
    class SYMBOL_EXPORT DepthFrame : public Frame
    {
    public:
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            DepthFrame *d = static_cast<DepthFrame *>(other.get());
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const DepthFrame *f = static_cast<const DepthFrame *>(&other);
            return f;
        }
    ...
    class SYMBOL_EXPORT RawFrame : public Frame
    {
    public:
    ...
    class SYMBOL_EXPORT ToFRawFrame : public RawFrame
    {
    public:
    ...
    class SYMBOL_EXPORT ToFRawFrameTemplate : public ToFRawFrame
    {
    public:
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *t = static_cast<ToFRawFrameTemplate<PhaseByteType, AmbientByteType> *>(other.get());
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const ToFRawFrameTemplate<PhaseByteType, AmbientByteType> *f = static_cast<const ToFRawFrameTemplate<PhaseByteType, AmbientByteType> *>(&other);
            return f;
        }
    ...
    class SYMBOL_EXPORT ToFRawIQFrame : public RawFrame
    {
    public:
    ...
    class SYMBOL_EXPORT ToFRawIQFrameTemplate : public ToFRawIQFrame
    {
    public:
        Vector<ByteType> _i;
        Vector<ByteType> _q;
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *t = static_cast<ToFRawIQFrameTemplate<ByteType> *>(other.get());
            t->id = id;
            t->type = type;
            t->timestamp = timestamp;
            t->_i = _i;
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const ToFRawIQFrameTemplate<ByteType> *f = static_cast<const ToFRawIQFrameTemplate<ByteType> *>(&other);
            return f;
        }
    ...
    class SYMBOL_EXPORT RawDataFrame : public RawFrame
    {
    public:
        float tsensor = -40.0f;
        // int tillum = -40;
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *r = static_cast<RawDataFrame *>(other.get());
            r->id = id;
            r->type = type;
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const RawDataFrame *f = static_cast<const RawDataFrame *>(&other);
            return f;
        }
        ...
        {
            return std::static_pointer_cast<RawDataFrame>(ptr);
        }
...
#else
    class SYMBOL_EXPORT RawDataFrame : public Frame
    {
    public:
        int tsensor = -40;
        int tillum = -40;
    ...
    class SYMBOL_EXPORT RawDataFrame_V2 : public RawDataFrame
    {
    public:
        int type = 0;
        virtual Ptr<Frame> copy() const
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *r = static_cast<RawDataFrame_V2 *>(other.get());
            r->id = id;
            r->type = type;
            r->timestamp = timestamp;
            r->tsensor = tsensor;
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const RawDataFrame_V2 *f = static_cast<const RawDataFrame_V2 *>(&other);
            return f;
        }
    ...
    class SYMBOL_EXPORT PointCloudFrame : public Frame
    {
    public:
...
#if defined (ANDROID) or defined (QNX)
    class SYMBOL_EXPORT PointCloudFrameTemplate : public Frame // PointCloudFrame
    {
    public:
        ...
        virtual Ptr<Frame> copyTo(Ptr<Frame> &other) const
        {
            ...
            auto *p = static_cast<PointCloudFrameTemplate<PointType> *>(other.get());
            p->id = id;
            p->type = type;
        ...
        virtual bool isSameType(const Frame &other) const
        {
            const PointCloudFrameTemplate<PointType> *f = static_cast<const PointCloudFrameTemplate<PointType> *>(&other);
            return f;
        }
        ...
        virtual bool serialize(SerializedObject &object) const
        {
            size_t s = sizeof(id) + sizeof(type) + sizeof(timestamp) + points.size() * sizeof(PointType) + sizeof(size_t);
        ...
        {
            return std::static_pointer_cast<PointCloudFrameTemplate<PointType>>(ptr);
        }

        virtual ~PointCloudFrameTemplate() {}
    };
#else
    class SYMBOL_EXPORT PointCloudFrameTemplate : public PointCloudFrame
    {
    public:
        Vector<PointType> points;
    ...
    template class SYMBOL_EXPORT PointCloudFrameTemplate<Point>;
    template class SYMBOL_EXPORT PointCloudFrameTemplate<IntensityPoint>;

    // template class SYMBOL_EXPORT PhaAmpFrame<uint16_t>;
    template class SYMBOL_EXPORT ToFRawFrameTemplate<uint16_t, uint8_t>;
    template class SYMBOL_EXPORT ToFRawIQFrameTemplate<int16_t>;
#endif
    /**
     * @}
     */
