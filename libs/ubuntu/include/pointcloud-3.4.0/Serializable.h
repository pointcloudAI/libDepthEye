...
#ifndef POINTCLOUD_SERIALIZABLE_H
#define POINTCLOUD_SERIALIZABLE_H
...
namespace std {
  template <>
  struct hash<PointCloud::SerializableString> {
    std::size_t operator()(const PointCloud::SerializableString &k) const
    {
...
public:
  ...
  virtual bool read(SerializedObject &object)
  {
    ...
    while(object.currentGetOffset() - g < size)
    {
      if(!k.read(object))
      {
          logger(LOG_ERROR) << "SerializableMap: read key '" << k << "' failed" << std::endl;
          continue;
          //return false;
      }
      auto x = this->find(k);
