...
{
	struct SYMBOL_EXPORT FrameStreamHeader
	{
	...
	struct SYMBOL_EXPORT FrameStreamPacket : public DataPacket
	{
		enum PacketType
		{
			PACKET_DATA = 0,
	...
	struct SYMBOL_EXPORT GeneratorConfigurationSubPacket
	{
		uint8_t frameType;
		uint32_t size;
	...
	class SYMBOL_EXPORT FrameStreamWriter
	{
	...
	public:
		...
		FrameStreamWriter( GeneratorIDType processedRawFrameGeneratorID,
                          GeneratorIDType depthFrameGeneratorID,
                          GeneratorIDType pointCloudFrameGeneratorID);
	...
	class SYMBOL_EXPORT FrameStreamReader
	{
...
// #if defined (ANDROID) || defined (QNX)
        BufferFramePtr          _bufferFrame;
        BufferFramePtr          _phaampFrame;
        VerticalDepthFramePtr   _outputFrame;
        TofConfSetting*         _confSetting;
        bool                _isVerticalDepth;
// #endif
		bool _init();
	...
	public:
		...
		FrameStreamReader(const String &fileName);
		FrameStreamReader(InputFileStream &stream);
        FrameStreamReader();
        ...
        bool getVerticalDepthFrame(VerticalDepthFramePtr& outFrame);
        bool getPhaAmpFrame(BufferFramePtr& outFrame);
        bool getRawDataFrame(BufferFramePtr& outFrame);
        void setVerticalOutput(bool enable);
		bool close();
	...
	// class SYMBOL_EXPORT SocketStreamReader
	// {
	// 	// CameraSystem &_sys;
	// 	FrameGeneratorPtr _frameGenerator[3]; // for processed raw, depth and point cloud
	...
	// 	SocketStreamReader(uint id1, uint id2, uint id3, char *config1, int len1, char *config2, int len2, char *config3, int len3);
