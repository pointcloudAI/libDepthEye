...
class POINTCLOUD_EXPORT ToFPhaAmpRawFrame : public ToFRawFrame
{
public:
    Vector<PhaseByteType> _pha_amp;
    ...
    {
        return (const uint8_t *)(_pha_amp.data() + size.height*size.width);
    }
    
    virtual uint8_t *amplitude()
    {
        return (uint8_t *)(_pha_amp.data() + size.height*size.width);
    }
    ...
    {
        return (const uint8_t *)_pha_amp.data();
    }
    
    virtual uint8_t *phase()
    {
        return (uint8_t *)_pha_amp.data();
    }
    ...
    virtual Ptr<Frame> copy() const
    {
        Ptr<Frame> f(new ToFPhaAmpRawFrame<PhaseByteType>());
        return copyTo(f);
    }
    ...
    {
        if(!other || !isSameType(*other))
            other = Ptr<Frame>(new ToFPhaAmpRawFrame<PhaseByteType>());
        
        auto *t = dynamic_cast<ToFPhaAmpRawFrame<PhaseByteType> *>(other.get());
    ...
    virtual Ptr<Frame> newFrame() const
    {
        ToFPhaAmpRawFrame<PhaseByteType> *t = new ToFPhaAmpRawFrame<PhaseByteType>();
        t->_pha_amp.resize(_pha_amp.size());
        t->size = size;
        return FramePtr(t);
    }
    ...
    virtual bool isSameType(const Frame &other) const
    {
        const ToFPhaAmpRawFrame<PhaseByteType> *f = dynamic_cast<const ToFPhaAmpRawFrame<PhaseByteType> *>(&other);
        return f;
    }
    
    static Ptr<ToFPhaAmpRawFrame<PhaseByteType> > typeCast(FramePtr ptr)
    {
        return std::dynamic_pointer_cast<ToFPhaAmpRawFrame<PhaseByteType> >(ptr);
    }
    ...
    virtual bool serialize(SerializedObject &object) const
    {
        ...
        object.put((const char *)&id, sizeof(id));
        object.put((const char *)&timestamp, sizeof(timestamp));
        object.put((const char *)_pha_amp.data(), sizeof(uint16_t)*_pha_amp.size());
    ...
    virtual bool deserialize(SerializedObject &object)
    {
        ...
        _pha_amp.resize(frameSize);
        
        object.get((char *)_pha_amp.data(), sizeof(uint16_t)*_pha_amp.size());
        return true;
    }
    
    virtual ~ToFPhaAmpRawFrame() {}
};

typedef Ptr<ToFPhaAmpRawFrame<uint16_t> > ToFPhaAmpRawFramePtr;
...
template class POINTCLOUD_EXPORT ToFPhaAmpRawFrame<uint16_t>;
