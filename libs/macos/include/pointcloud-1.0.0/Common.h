...
void POINTCLOUD_EXPORT breakLines(const String &str, std::ostream &out, const uint maxPerLine, const String &newlinePrefix);
...
uint POINTCLOUD_EXPORT gcd(uint n, uint m);

// This returns nearest 'n = 2^m' such that 2^(m - 1) < value < 2^m
unsigned int POINTCLOUD_EXPORT nearestPowerOf2(unsigned int value, unsigned int &index);
...
int POINTCLOUD_EXPORT getFiles(const String &dir, const String &matchString, Vector<String> &files);

bool POINTCLOUD_EXPORT isFilePresent(const String &filename);
bool POINTCLOUD_EXPORT isDirectory(const String &filename);
bool POINTCLOUD_EXPORT makeDirectory(const String &filename);
String POINTCLOUD_EXPORT dirname(const String &filename);
String POINTCLOUD_EXPORT basename(const String &filename);

String POINTCLOUD_EXPORT substitute(const String &s, const Vector<String> &keys, const Vector<String> &values);
