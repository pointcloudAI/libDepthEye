...
#ifndef POINTCLOUD_FRAMEBUFFER_H
#define POINTCLOUD_FRAMEBUFFER_H
...
public:
  FrameBuffer(BufferPtr &buffer, FrameBufferManager<BufferType> &manager):
    Ptr<BufferType>(nullptr, [&manager, &buffer](BufferType *) { manager.release(buffer); }), _buffer(buffer),_manager(manager) // The deleter gets called when FrameBuffer<> goes out of scope. Deleter releases the held buffer
    {}
